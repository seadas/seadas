...
public class OCSSWRemote {
    ...
    public Process execute(String[] commandArray) {
        ...
        try {
            ...
            if (process != null) {
                ServerSideFileUtilities.debug("Running the program " + commandArray.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private String getFullIFileName(String ifileName) {
        String workingDirectory = System.getProperty(SERVER_WORKING_DIRECTORY_PROPERTY, DEFAULT_WORKING_DIR );
        return workingDirectory + File.separator + ifileName;
    }

    private void extractFileInfo(String ifileName, String jobId){
    ...
    private void selectExtractorProgram(String jobId) {
        ...
        SQLiteJDBC.updateItem(SQLiteJDBC.FILE_TABLE_NAME, jobId, SQLiteJDBC.PROGRAM_NAME_FIELD_NAME, programName);
    }
