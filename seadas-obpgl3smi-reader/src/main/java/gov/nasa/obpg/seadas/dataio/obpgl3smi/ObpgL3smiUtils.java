...
public class ObpgL3smiUtils {
    ...
    public Product createL3SmiProductBody(List<Attribute> globalAttributes) throws ProductIOException {
        //String title = getStringAttribute("Title", globalAttributeList);
        String productName = getStringAttribute(KEY_NAME, globalAttributes);
        String productType = "OBPG " + getStringAttribute(KEY_TYPE, globalAttributes);

        int sceneWidth = getIntAttribute(KEY_L3SMI_WIDTH, globalAttributes);
        int sceneHeight = getIntAttribute(KEY_L3SMI_HEIGHT, globalAttributes);
    ...
    public void addL3SmiScientificMetadata(Product product, NetcdfFile ncFile) throws IOException {
        //final MetadataElement scanLineAttrib = getMetadataElementSave(product, );
    ...
    private void computeLatLonBandData(final Band latBand, final Band lonBand,
        ...
        if (mustFlip) {
            ObpgL3smiProductReader.reverse(latFloats);
            ObpgL3smiProductReader.reverse(lonFloats);
        }
