...
public class CloProgramUIImpl extends JPanel implements CloProgramUI {
    ...
    public ProcessorModel getProcessorModel() {
        ...
        if (programName.indexOf("smigen") != -1 && smitoppmCheckBox.isSelected()) {
            ...
            ParamInfo pi1 = new ParamInfo("ifile", processorModel.getParamValue(processorModel.getPrimaryOutputFileOptionName()));
            pi1.setOrder(1);
            ParamInfo pi2 = new ParamInfo("ofile", ppmFile.getFileName());
            pi2.setOrder(2);
            smitoppm.addParamInfo(pi1);
            smitoppm.addParamInfo(pi2);
            processorModel.setSecondaryProcessor(smitoppm);
        }
...
//                                                    outputFileSelector.getModel().setProductName(outputFile.getName());
    ...
    private JPanel createOutputFilePanel() {
        ...
        ctx.addPropertyChangeListener(primaryOutputFileOptionName, new PropertyChangeListener() {
            ...
            @Override
            public void propertyChange(PropertyChangeEvent pce) {
                System.out.println("ctx ofile: " + primaryOutputFileOptionName + " + " + ofileTextField.getText() + " " + outputFileSelector.getFileName());
                processorModel.updateParamInfo(primaryOutputFileOptionName, outputFileSelector.getFileName());
            }
        });
